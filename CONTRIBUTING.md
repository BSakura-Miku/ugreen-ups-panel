...
采集器更新变更还需覆盖错误密钥、同源限制、请求/归档大小、固定发行来源、SHA及源码指纹、链接与路径穿越、并发版本改变、更新失败恢复、状态持久化和宿主权限边界。使用临时目录及模拟 systemd 测试，不在真实 UPS 上注入损坏版本。
...
面板镜像发布到 Docker Hub，Compose 和采集器安装脚本随 GitHub 仓库提供。GitHub Release 记录版本变化、镜像标签、升级步骤与验证结果；v0.9.0 起为网页更新提供最小采集器代码包与 SHA256SUMS，不额外制作完整源码或部署压缩包。已经发布的旧版附件保留，避免破坏历史下载链接。
...
新增公开文件时同步更新检查清单。检查脚本只读文件，CI 直接在仓库中运行测试、验证 Compose、构建镜像并检查容器启动及接口，默认检查不生成发布包。采集器发行附件使用下面的专用打包器。清单与基础敏感信息扫描不能替代人工核查、素材授权检查或 Git 历史检查。
...
3. 从同一提交执行 `python3 scripts/package-collector.py --output-dir /绝对临时目录/collector-release`，生成 `collector-v版本.tar.gz` 与 `SHA256SUMS`；打包器记录该提交和源码指纹，拒绝覆盖已有文件。使用 `ups_panel.update_release.inspect_package` 回读制品，再上传到对应正式 GitHub Release。
4. 在同一提交创建 Git 标签和 GitHub Release，说明变更、镜像 digest、升级步骤和验证范围。网页更新包仅含 Python 代码、固定元数据与 LICENSE，不包含安装脚本、systemd 配置或数据。若变更需要宿主配置或更新服务自身升级，必须明确给出命令行步骤。

首次安装按 README 下载仓库源码、安装宿主机采集器，再在 Docker 中创建项目。安装脚本准备数据目录，Compose 自动拉取镜像。需要升级采集器时，用 GitHub 对应版本的自动源码归档在临时目录展开，安装器通过 `--data-dir` 指向已有部署数据目录，安装后清理临时源码。启用网页更新的用户使用正式发行的最小采集器包；其信任、格式与恢复边界见[更新说明](docs/collector-update.md)。日常面板更新执行 `docker compose pull && docker compose up -d`；数据库继续保留在 `./data`。
