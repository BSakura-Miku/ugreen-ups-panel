...
function SessionStatus({ record, fresh }: { record: BatterySession; fresh: boolean }) {
  const endNote = record.end_reason === 'offline' ? '采集中断' : record.end_reason === 'gap' ? '记录间断' : record.end_reason === 'unknown' ? '结束原因未确认' : '';
...
function BatterySessionRecords({ days, onDaysChange }: { days: number; onDaysChange: (days: number) => void }) {
  ...
  return <article className="panel battery-sessions" aria-labelledby="battery-sessions-heading">
    <div className="panel-heading"><div><h3 id="battery-sessions-heading">电池供电记录</h3><p>查看已记录的供电切换、持续时长与电量变化。</p></div><span className="muted">时间范围独立于运行趋势</span></div>
    ...
    {loading ? <p className="session-empty" role="status">正在加载电池供电记录…</p> : <>
      ...
      {data && summary && <>
        ...
        <div className="session-scope"><p>{isFiniteNumber(data.recording_since) ? `开始记录于 ${historyDate(data.recording_since)}（本地时间），此前的供电过程不补算。` : '尚未开始记录，收到有效采集数据后自动记录。'}</p><p>次数只表示已记录且确认的供电切换，不是电池循环次数。进行中的时长只计到最近一次采样；负下降值表示终点电量高于起点。</p>{!data.capture_fresh && <p className="session-capture-note">采集暂不可用，时长和电量仅截至最后一次记录。</p>}</div>
        {data.records.length ? <>
          <p className="session-record-count">当前范围共 {data.total_records} 条记录{data.has_more ? `，显示最近 ${data.records.length} 条` : ''}。跨越边界的行保留该次完整观测，汇总时长只计所选范围。</p>
          <table className="session-table"><caption className="visually-hidden">电池供电逐次记录，时间按本地显示</caption><thead><tr><th scope="col">开始</th><th scope="col">结束 / 最近观测</th><th scope="col">持续时长</th><th scope="col">电量起止</th><th scope="col">电量变化</th><th scope="col">记录状态</th></tr></thead><tbody>{data.records.map(record => <tr key={record.id}>
...
function App() {
  ...
  return <div className="shell">
    ...
    <main id="main-content" tabIndex={-1}><header><div className="breadcrumb">设备监控 <ChevronRight size={14} /><span>US3000</span></div><div className="header-status"><span className={`dot ${fresh ? 'green' : 'amber'}`} />{fresh ? '实时采集' : apiError ? '连接中断' : view ? '等待数据' : '连接中'}<span className="header-clock">{new Date(clock).toLocaleTimeString('zh-CN', { hour12: false })}</span></div></header>
      <div className="page-heading"><div><div className="eyebrow">UGREEN US3000</div><h1>{tab === 'overview' ? '电力概览' : tab === 'hardware' ? '硬件档案' : tab === 'calibration' ? '功率校准' : '数据说明'}</h1><p>{tab === 'calibration' ? '查看当前系数，按实际测量调整估算。' : '关注每一次供电，了解每一节电芯。'}</p></div><div className="serial">USB 直连<span>{view?.device?.serial || 'US3000'}</span></div></div>
      ...
      {tab === 'overview' ? <>
        ...
        <section className="detail-grid"><article className="panel trend">
          <div className="panel-heading"><div><h3>运行趋势 <span>{unit}</span></h3><p>{metric === 'ac_input_estimate_w' ? '交流输入估算 · 电池供电期间留空' : metric === 'battery_energy_estimate_w' ? '电池端估算 · 外部供电期间留空' : metric === 'cell_delta_mv' ? '同时查看压差均值与峰值 · 中断时段留空' : '历史趋势 · 中断时段留空'}</p></div></div>
