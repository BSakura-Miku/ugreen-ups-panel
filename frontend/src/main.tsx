...
import BatteryCapacityCard from './BatteryCapacityCard';
import EnergyUsageCard from './EnergyUsageCard';
import { cellBalancePresentation } from './batteryDisplay';
import DiagnosticsPanel from './DiagnosticsPanel';
import { historyPollInterval, startVisiblePolling } from './readPolling';
...
function historySummary(history: History) {
  if (typeof history.available_start !== 'number' || !Number.isFinite(history.available_start)
      || typeof history.available_end !== 'number' || !Number.isFinite(history.available_end)) return '暂无已保存记录 · 统计说明';
  const start = new Date(history.available_start * 1000), end = new Date(history.available_end * 1000);
  const showYear = start.getFullYear() !== end.getFullYear() || end.getFullYear() !== new Date().getFullYear();
  const compact = (date: Date) => date.toLocaleString('zh-CN', {
    ...(showYear ? { year: 'numeric' } as const : {}), month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false,
  });
  return `${history.resolution_sec >= 86400 ? 'UTC 日统计 · ' : ''}已记录 ${compact(start)}–${compact(end)}（本地）· 统计说明`;
}
...
function BatterySessionRecords({ days, onDaysChange }: { days: number; onDaysChange: (days: number) => void }) {
  ...
  useEffect(() => {
    ...
    return startVisiblePolling(async signal => {
      try {
        const next = await fetchJson<BatterySessions>(`/api/battery-sessions?days=${days}&limit=50`, signal);
  ...
  return <article className={`panel battery-sessions ${data?.records.length ? 'has-records' : ''}`} aria-labelledby="battery-sessions-heading">
    <div className="panel-heading"><div><h3 id="battery-sessions-heading">电池供电记录</h3><p>供电切换、时长与能量估算 · 独立时间范围</p></div></div>
...
function App() {
  ...
  const isBattery = !!fresh && s?.mode === 'battery';
  const currentSample = fresh ? s : null;
  const supplyKnown = !!fresh && ['online', 'charging', 'battery'].includes(s?.mode || '');
  const cellPresentation = cellBalancePresentation(view?.cell_balance, s?.mode, !!fresh);
  const cellWarning = cellPresentation.tone === 'elevated' || cellPresentation.tone === 'check';
  ...
  return <div className="shell">
    ...
    <main id="main-content" tabIndex={-1}><header><div className="breadcrumb">设备监控 <ChevronRight size={14} /><span>US3000</span></div><div className="header-status">{isBattery && <span className="header-battery"><AlertTriangle size={15}/>UPS 电池供电</span>}<span className={`dot ${fresh ? 'green' : 'amber'}`} />{fresh ? '实时采集' : apiError ? '连接中断' : view ? '等待数据' : '连接中'}<span className="header-clock">{new Date(clock).toLocaleTimeString('zh-CN', { hour12: false })}</span></div></header>
      ...
      {tab === 'overview' ? <>
        <section className={`panel overview-summary ${isBattery ? 'summary-on-battery' : ''}`} aria-label="实时供电与读数">
          ...
          <div className="primary-readings">
            ...
            <Metric icon={<Zap size={16}/>} label="交流输入功率 · 估算" value={num(currentSample?.ac_input_estimate_w)} unit="W" note={powerNote(currentSample, 'ac')}/>
            <Metric icon={<BatteryCharging size={16}/>} label="电池放电功率 · 估算" value={num(currentSample?.battery_energy_estimate_w)} unit="W" note={powerNote(currentSample, 'battery')}/>
            <Metric icon={<BatteryCharging size={16}/>} label="电池组电压" value={num(currentSample?.battery_voltage, 3)} unit="V" note="额定电池能量 43.2 Wh"/>
          </div>
          <div className="power-caption"><span>两项功率均非 NAS 输出功率。<a href="#diagnostics">估算依据 ↗</a></span><a className="power-calibration-link" href="#calibration"><SlidersHorizontal size={14}/><span>{fresh ? '当前配置' : '最近配置'}：{calibrationLabel(s?.calibration_profile)} · 调整系数</span><ChevronRight size={14}/></a></div>
      ...
      <footer><span><ShieldCheck size={14} />面板不执行关机或 UPS 控制</span><span>US3000 Monitor · v0.12.0</span></footer>
    </main>
  </div>;
}
