...
import type { CalibrationState, CalibrationVoltage, LiveView } from './types';
import { applySuggestion, calibrationPayload, chooseVoltage, coefficientSource, confirmVoltage, createStateDraft, chooseProfile, editCoefficient, formatCoefficient, presetDraft } from './calibrationDraft';
...
import { calibrationCanSave, calibrationEditRevision, calibrationLabel, calibrationReadinessMessage, parseCalibrationState, usableCalibrationConfig } from './calibrationState';
...
async function requestCalibration(parentSignal: AbortSignal, init?: RequestInit): Promise<CalibrationState> {
  ...
  try {
    ...
    const state = parseCalibrationState(body);
    if (!state) throw new CalibrationRequestError('配置响应无效，请稍后重试。');
    return state;
  } catch (error) {
...
export default function CalibrationSettings({ live, liveFresh, liveAge }: { live: LiveView | null; liveFresh: boolean; liveAge: number | null }) {
    ...
    async function update() {
      ...
      try {
        ...
        if (!stopped && version === requestVersion.current) {
          ...
          if (!initialized.current) { setDraft(createStateDraft(next)); initialized.current = true; }
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (!supportsV2 || !liveFresh || draft?.voltage !== null || draft.profile === null) return;
  ...
  function fillAssistant(suggestion: AssistantSuggestion) {
    if (!draft || !data || draft.profile === null || !supportsV2 || !calibrationCanSave(data) || busyRef.current || conflict || calibrationEditRevision(data) !== draft.revision || pollError || receivedAt === null || performance.now() - receivedAt > 10000 || !liveFresh || liveAge === null || liveAge > 5) return;
  ...
  async function reload() {
    ...
    try {
      ...
      setData(next); setReceivedAt(performance.now()); setDraft(createStateDraft(next)); initialized.current = true;
  ...
  function reset() {
    ...
    setDraft(createStateDraft(data)); setDirty(false); setActionError(''); setConflict(false); setAssistantReset(value => value + 1); setFillNotice('');
  }

  async function save(event: FormEvent) {
    event.preventDefault();
    if (!draft || !data || !calibrationCanSave(data) || busyRef.current || conflict || calibrationEditRevision(data) !== draft.revision || pollError || receivedAt === null || performance.now() - receivedAt > 10000) return;
    ...
    try {
      ...
      if (lifetime.current.signal.aborted) return;
      setData(next); setReceivedAt(performance.now()); setDraft(createStateDraft(next)); setDirty(false); setConflict(false); setPollError('');
      setSavedRevision(calibrationEditRevision(next)); setFillNotice(''); setAssistantReset(value => value + 1);
    } catch (error) {
  ...
  const active = usableCalibrationConfig(data?.active) ? data.active : null;
  const reportedActive = data?.active;
  const reportFresh = !pollError && receivedAt !== null && Math.max(0, clock - receivedAt) <= 10000;
  const collectorReady = reportFresh && !!data?.collector_ready && !!active && (data.readiness?.ready ?? true);
  const canSave = reportFresh && calibrationCanSave(data);
  const readinessMessage = calibrationReadinessMessage(data);
  const changedElsewhere = !!data && !!draft && calibrationEditRevision(data) !== draft.revision;
  const confirmed = !!savedRevision && collectorReady && data?.active?.revision === savedRevision && calibrationEditRevision(data) === savedRevision;
  const replaced = !!savedRevision && !!data && calibrationEditRevision(data) !== savedRevision;
  const assistantEnabled = canSave && draft?.profile !== null && supportsV2 && !busy && !conflict && !changedElsewhere;
  const waiting = !!data && !data.configuration_problem && usableCalibrationConfig(data.desired) && (data.pending || (!!savedRevision && data.active?.revision !== savedRevision && !replaced));
  const saveBlockReason = !data ? '正在读取校准配置。' : !reportFresh ? '配置连接中断或报告已过期，请等待刷新或重新载入。'
    : conflict || changedElsewhere ? '配置已发生变化，请重新载入后再保存。' : !canSave ? readinessMessage || '校准配置尚未就绪，请查看上方检查结果。'
    : draft?.profile === null ? draft.configurationIssue || '请先选择支持的校准方式。' : draft?.profile === 'custom' && draft.schema === 2 && !supportsV2 ? '当前采集器不支持电压档位，请先更新采集器。'
    : !dirty ? '尚无未保存的修改。' : '';
  const assistantBlockReason = !canSave ? readinessMessage || saveBlockReason : conflict || changedElsewhere ? '配置已发生变化，请重新载入。' : draft?.profile === null ? '请先在下方选择支持的校准方式；12 V 适配器使用自定义系数。' : busy ? '正在处理配置，请稍候。' : '';
  return <section className="calibration-page" aria-label="功率校准设置">
    <article className="panel calibration-active">
      ...
      {data && reportFresh && readinessMessage && <p className="notice" role="status">{readinessMessage}{canSave && !collectorReady ? ' 可以在下方重新配置并保存。' : !canSave ? ' 暂不能保存。' : ''}</p>}
      {reportedActive?.schema === null && <p className="notice" role="status">{reportedActive.invalid_reason} 此配置的系数与电压范围不作展示。</p>}
      {data?.error && data.error !== readinessMessage && <p className="notice" role="status">{data.error}</p>}
    ...
    <article className="panel calibration-editor">
      ...
      {draft && data ? <form onSubmit={save} noValidate>
        <fieldset disabled={!!busy} className="calibration-fields">
          ...
          <select id="calibration-profile" value={draft.profile ?? 'unsupported'} onChange={event => { edit(chooseProfile(draft, event.target.value)); setAssistantReset(value => value + 1); }}>
            {draft.profile === null && <option value="unsupported" disabled>当前配置无效或不受支持，请选择</option>}
            <option value="none">未启用</option><option value="local-19v-v1">开发样机 · 19 V v1</option><option value="custom">自定义系数</option>
          </select>
          <p className="muted">{draft.profile === null ? draft.configurationIssue : draft.profile === 'none' ? '保存后停止功率估算，电量、电压和设备电流继续显示。' : draft.profile === 'custom' ? '自定义系数未独立验证。下方为待保存数值，保存并经采集器确认后才会生效。' : '使用开发样机的 19 V 经验参数；相同型号也可能存在差异。保存并经采集器确认后才会生效。'}</p>
          <p className="muted">12 V 适配器：选择“自定义系数”，确认 12 V 档位后填写本机系数或使用校准助手。</p>
          ...
          {(draft.profile === 'custom' || draft.profile === 'local-19v-v1') && <div className="calibration-inputs">{fields.map(field => <div className="calibration-field" key={field.key}>
        ...
        {saveBlockReason && <p id="calibration-save-reason" className="muted" role="status">保存暂不可用：{saveBlockReason}</p>}
        <div className="calibration-actions calibration-save-actions">
          <button className="calibration-primary" type="submit" aria-describedby="calibration-save-reason" disabled={!!busy || !!saveBlockReason}>{busy === 'save' ? '正在保存…' : '保存配置'}</button>
