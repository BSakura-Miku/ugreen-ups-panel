...
import { COLLECTOR_UPDATE_DOCS, collectorAdminKeyReady, collectorOperationResult, collectorReleaseTarget, collectorReleaseUrl,
  ...
  parseCollectorUpdateStatus, sameCollectorTarget } from './collectorUpdate';
...
function CollectorUpdateContent({ fallbackCurrent }: { fallbackCurrent?: DiagnosticBuild | null }) {
  ...
  return <article className="panel update-panel" aria-labelledby="collector-update-heading">
    ...
    <dl className="update-versions"><div><dt>当前采集器</dt><dd>{diagnosticVersion(currentVersion || fallbackCurrent?.version)}</dd></div><div><dt>最新稳定版</dt><dd>{status?.latest?.version || '尚未检查'}</dd></div></dl>
...
export default function CollectorUpdatePanel(props: { fallbackCurrent?: DiagnosticBuild | null }) {
