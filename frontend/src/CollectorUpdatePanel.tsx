...
import { AlertTriangle, ArrowDownToLine, ArrowUpRight, CheckCircle2, Eye, EyeOff, KeyRound, RotateCcw, X } from 'lucide-react';
...
function CollectorUpdateContent({ fallbackCurrent, diagnosticsFresh = false, captureFresh = false, checks }: Props) {
  ...
  useEffect(() => {
    ...
    return startVisiblePolling(async signal => {
      ...
      try {
        let next = parseCollectorUpdateStatus(await readJson('/api/collector-update', signal));
        if (next?.automatic_check?.due && !collectorUpdateBusy(next)) {
          const request = collectorUpdateRequest('check', '');
        ...
        if (!signal.aborted && epoch === mutationEpoch.current) {
          knownBusy = collectorUpdateBusy(next) || next.automatic_check?.busy === true;
  ...
  return <article className="panel update-panel" aria-labelledby="collector-update-heading">
    ...
    {status?.automatic_check?.busy && <p className="update-note" role="status">正在联网检查最新稳定版…</p>}
    {status?.automatic_check?.error && <p className="update-feedback" role="status">{collectorUpdateError(status.automatic_check.error.code)}{status.latest ? ' 保留上次检查结果，' : ' '}5 分钟后自动重试。</p>}
    {controlsAvailable && !status?.automatic_check?.supported && <p className="update-feedback">宿主更新服务需要升级后才能自动检查版本。</p>}
