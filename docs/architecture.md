...
v0.12.0 在原有后台记录器中增加市电用电积分。有效相邻交流输入估算读数按时间累计，按北京时间小时和当时依据分段保存，日历与月累计共用同一账本。写入与积分游标随历史事务提交；只读接口合并已保存与待提交记录，不推进采样。该账本不随趋势清理，旧聚合不回填，详见[用电统计](energy-usage.md)。
...
v0.11.0 在同一数据库以独立表保存相对容量参考及重建前的阶段摘要。参考状态与采样历史使用同一写入事务；API 读取不推进采样。当前基线和已归档阶段不随一年趋势数据清理而移除，旧版本可继续使用原有历史表。降级再升级后不会拼接未完成的容量区间，见[相对容量参考](battery-capacity.md)。
...
分层保留规模、索引、查询实现与一年模拟数据实测见[存储与性能](STORAGE.md)。
...
v0.7.0 为新记录追加 `energy` 对象，包含未平滑电池功率的梯形积分、有效覆盖时长/比例及依据；旧记录为 `null`，不从聚合均值回填。能量、积分锚点和原供电明细在同一事务中提交，保留原明细表列结构及数据库版本；来源、设备或有效校准改变后分段，失效点和超过 5 秒的间隔不参与积分。覆盖率以首末电池采样间的时长为分母，与原供电明细包含恢复外电边界的时长分开。

`GET /api/live` 增加 `cell_balance` 参考观察结果。后台采样推进内存中的待机计时与等级持续时间，GET 不推进观察；数据库不可写也不停止压差观察。服务重启、采集中断或设备变更后重新计时。具体区间、时长、依据和边界见[电池观察说明](battery-observation.md)。
